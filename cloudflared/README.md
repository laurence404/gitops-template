...
Example usage for connecting to Kubernetes API (sets up `127.0.0.1:6443` to forward via Cloudflare to `kubernetes.default:443` within your cluster - you'll need to update `~/.kube/config` to contain `server: https://127.0.0.1:6443`) then run:
```
cloudflared access tcp --hostname bastion.yourdomain.com --listener 127.0.0.1:6443 --destination kubernetes.default:443
kubectl get pods
```
This will bring up a web browser to authenticate (Note: certificate autentication doesn't appear to work in this case). Alternatively, create service tokens in Cloudflare Zero Trust -> Access -> Service auth, and add the following parameters: `--service-token-id <CF-Access-Client-Id> --service-token-secret <CF-Access-Client-Secret>`
